...
## Run the unit tests
unit-test:
	source venv/bin/activate && PYTHONPATH=${PYTHONPATH} pytest -p no:logging || true

check-coverage:
	source venv/bin/activate && PYTHONPATH=${PYTHONPATH} coverage run --omit 'venv/*' -m pytest -p no:logging && coverage report -m || true
