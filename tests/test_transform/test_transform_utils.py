...
class TestReadCsvToDf:
    ...
    def test_single_key_list_yields_single_df(self, aws_credentials):
        ...
        s3_client.put_object(
            Bucket=self.TEST_BUCKET, Key="test_object_1.csv", Body=self.CSV_1
        )
    ...
    def test_multiple_key_list_yields_multiple_dfs(self, aws_credentials):
        ...
        s3_client.put_object(
            Bucket=self.TEST_BUCKET, Key="test_object_2.csv", Body=self.CSV_2
        )
        ...
        expected_df_2 = pd.DataFrame(
            {
                "address2_id": [1],
                "address2_line_1": ["12 Gold Close"],
                "address_line2_2": ["Harlow"],
            }
        )
    ...
    def test_function_raises_name_error(self, aws_credentials, caplog):
        ...
        with caplog.at_level(logging.ERROR):  # Capture logs at ERROR level
            ...
            assert (
                f"The key '{test_list[0]}' does not exist in bucket '{self.TEST_BUCKET}'."
                in caplog.text
            )
